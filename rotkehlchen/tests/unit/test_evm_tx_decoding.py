...
def test_decode_pending_transactions_different_chains(
    ...
    dbevmtx = DBEvmTx(database)
    with database.user_write() as cursor:
        dbevmtx.add_evm_transactions(cursor, [transaction_opt, transaction_eth], relevant_address=None)  # noqa: E501

    assert len(dbevmtx.get_transaction_hashes_no_receipt(tx_filter_query=None, limit=None)) == 2
    eth_transactions.get_receipts_for_transactions_missing_them()
    assert dbevmtx.get_transaction_hashes_no_receipt(tx_filter_query=None, limit=None) == [evmhash_opt]  # noqa: E501
    optimism_transactions.get_receipts_for_transactions_missing_them()
    assert len(dbevmtx.get_transaction_hashes_no_receipt(tx_filter_query=None, limit=None)) == 0
