...
def get_underlying_asset_price(token: EvmToken) -> Optional[Price]:
    ...
    custom_token = GlobalDBHandler().get_evm_token(evm_token_address)
...
class Inquirer():
    ...
    def find_uniswap_v2_lp_price(
        ...
        try:
            ...
            log.error(
                ...
                f'token {evm_token_address} properties: {str(e)}',
            )
            return None
        ...
        for (method_output, method_name) in zip(output, methods):
            if method_output[0] and len(method_output[1]) != 0:
                ...
                log.debug(
                    ...
                    f'for token {evm_token_address}',
                )
                return None

        try:
            token0 = EvmToken(decoded[0])
            token1 = EvmToken(decoded[1])
        except UnknownAsset:
            return None
    ...
    def find_curve_pool_price(
        ...
        if lp_tokenevm_address not in pools:
            return None
        pool = pools[lp_tokenevm_address]
        tokens = []
    ...
    def find_yearn_price(
        ...
        maybe_underlying_token = GlobalDBHandler().fetch_underlying_tokens(evm_token_address)
        ...
        output = multicall_2(
            ethereum=self._ethereum,
            require_success=True,
            calls=[(evm_token_address, contract.encode(method_name='pricePerShare'))],
        )
